export default function NexusLogo({ className = "h-10 w-10", withWordmark = false }: { className?: string; withWordmark?: boolean }) {
  return (
    <span className="inline-flex items-center gap-2">
      ...
      {withWordmark && <span className="text-sm font-black tracking-[0.14em] text-white">NEXUS <span className="text-cyan-300">DROP</span></span>}
    </span>
  );
}

export function NexusWordmark() {
  return <span className="hidden items-center gap-2 lg:inline-flex"><NexusLogo className="h-8 w-8" /><span className="text-sm font-black tracking-[0.14em] text-white">NEXUS <span className="text-cyan-300">DROP</span></span></span>;
}
