...
export const appRouter = router({
  ...
  admin: router({
    ...
    productActivity: adminProcedure.input(z.object({ limit: z.number().int().min(1).max(50).optional(), administrator: z.string().max(120).optional(), action: z.enum(["created", "updated"]).optional(), from: z.string().date().optional(), to: z.string().date().optional() }).optional()).query(({ input, ctx }) => listProductActivity({ userId: ctx.user.id, limit: input?.limit, administrator: input?.administrator, action: input?.action, from: input?.from, to: input?.to })),
