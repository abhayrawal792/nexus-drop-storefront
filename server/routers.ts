...
export const appRouter = router({
  ...
  store: router({
    ...
    requestRestock: publicProcedure.input(z.object({ productId: z.string().uuid(), email: z.string().max(254) })).mutation(({ input, ctx }) => { const forwarded = ctx.req.headers["x-forwarded-for"]; const ipAddress = typeof forwarded === "string" ? forwarded.split(",")[0].trim() : ctx.req.socket.remoteAddress ?? "unknown"; return requestRestock({ ...input, ipAddress, userId: ctx.user?.id, userName: ctx.user?.name }); }),
    ...
    restockAlertHistory: protectedProcedure.query(({ ctx }) => listCustomerRestockHistory(ctx.user.id, ctx.user.email ?? "", ctx.user.name)),
  ...
  admin: router({
    ...
    analytics: adminProcedure.input(z.object({ from: z.string().date().optional(), to: z.string().date().optional(), attributionDays: z.number().int().min(1).max(30).optional(), productId: z.string().uuid().optional(), categoryId: z.string().uuid().optional(), failureType: z.enum(["provider", "invalid_recipient", "temporary", "unknown"]).optional(), failureProductId: z.string().uuid().optional() }).optional()).query(({ input }) => getAdminAnalytics(input ?? {})),
    ...
    productActivity: adminProcedure.input(z.object({ limit: z.number().int().min(1).max(50).optional(), page: z.number().int().min(1).optional(), pageSize: z.number().int().min(1).max(25).optional(), administrator: z.string().max(120).optional(), action: z.enum(["created", "updated"]).optional(), from: z.string().date().optional(), to: z.string().date().optional() }).optional()).query(({ input, ctx }) => listProductActivity({ userId: ctx.user.id, limit: input?.limit, page: input?.page, pageSize: input?.pageSize, administrator: input?.administrator, action: input?.action, from: input?.from, to: input?.to })),
